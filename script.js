...
function canvas1() {
    ...
    function files(index) {
        var data = `
./assets/canvas/bridges00004.png
...
./assets/canvas/bridges00106.png
...
`;
    ...
    function updatePercentage() {
        // Retrieve the ScrollTrigger instance for "#page7"
        const scrollTrigger = ScrollTrigger.getById("page7-trigger");

        if (scrollTrigger) {
            let scrollProgress = scrollTrigger.progress;
            // Cap the percentage at 60%
            const cappedProgress = Math.min(scrollProgress, 0.6);
            const scrollPercent = Math.round(cappedProgress * 100);
            document.querySelector('.percentage').textContent = `${scrollPercent}%`;
        }
    }
...
tl.to(".page7-cir", { width: "50vw", height: "50vw", duration: 3, ease: "power1.out" })
    .to(".page7-cir", { width: "25vw", height: "25vw", duration: 2, ease: "power1.in" });
